...
class WordRepositoryImpl(
    ...
    override suspend fun getWordList(word: String): Flow<Resource<List<WordModel>>> {
        return flow {
            ...
            val wordListCached = dictionaryDao.getWordList(word).map { it.toWordModel() }
            emit(Resource.Loading(data = wordListCached))
            ...
            result.collect { apiState ->
                when (apiState) {
                    ...
                    is ApiState.Error ->
                        emit(Resource.Error(apiState.message!!, data = wordListCached))
                    is ApiState.Success -> {
